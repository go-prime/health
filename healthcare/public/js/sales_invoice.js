...
var get_consumables_to_invoice = function(frm){
	...
	dialog.fields_dict["patient"].df.onchange = () => {
		...
		if(patient && patient!=selected_patient){
			...
			var columns = (["item_code", "item_name","reference_name", "reference_type", "qty" ]);
			frappe.call({
				method: "healthcare.healthcare.utils.get_consumables_to_invoice",
				args: {patient: patient, company: frm.doc.company},
			}).then(data => {
				if(data.message){
					$results.append(make_list_row(columns, true));
					for(let i=0; i<data.message.length; i++){
						$results.append(make_list_row(columns, true, data.message[i]));
					}
			};
		})
		}
	...
	$placeholder = $(`<div class="multiselect-empty-state">
				<span class="text-center" style="margin-top: -40px;">
					...
					<p class="text-extra-muted">No Consumables found</p>
				</span>
			</div>`);
...
var get_checked_values= function($results) {
	return $results.find('.list-item-container').map(function() {
		...
		if ($(this).find('.list-row-check:checkbox:checked').length > 0 ) {
			...
			if(checked_values['rate'] == null ||  checked_values['rate'] == "null" ){
				checked_values['rate'] = 0
			}
